...
def test_delete_model(key: int, bst: BST[int,int]) -> None:
    assume(is_valid(bst))
    abstract_data = bst.to_list()
    deleted_abstract_data = [(k, v) for (k, v) in abstract_data if k != key]
...
def test_union_model(bst1: BST[int,int], bst2: BST[int,int]) -> None:
    ...
    # Union: keys from bst1 take precedence over keys from bst2
    unioned_abstract_data = dict(abstract_data2)  # Start with bst2's data
    unioned_abstract_data.update(dict(abstract_data1))  # Overwrite with bst1's data
    unioned_bst = BST.union(bst1, bst2)
    assert set(unioned_bst.to_list()) == set(unioned_abstract_data.items())
