...
def test_delete_post(key: int, bst: BST[int,int], search_key: int) -> None:
    ...
    expected = None if key == search_key else bst.find(search_key)
    assert found == expected

# TODO: After merging, the lookup should return the correct value, with keys from bst1 taking precedence over keys from bst2.
@given(bst1=trees_strategy, bst2=trees_strategy, search_key=keys_strategy)
def test_union_post(bst1: BST[int,int], bst2: BST[int,int], search_key:int) -> None:
    assume(is_valid(bst1))
    assume(is_valid(bst2))
    
    # Force the key to exist in both trees with DIFFERENT values to detect the bug
    # This ensures we actually test the precedence rule
    bst1_enriched = bst1.insert(search_key, 999)  # bst1 gets value 999
    bst2_enriched = bst2.insert(search_key, 888)  # bst2 gets value 888
    
    union_bst: BST[int,int] = BST.union(bst1_enriched, bst2_enriched)
    found = union_bst.find(search_key)
    
    # According to spec: bst1 takes precedence, so should be 999
    # Bug will make it 888 (bst2's value)
    expected = 999
